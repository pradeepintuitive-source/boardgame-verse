...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [{ title: "Profile — GameHub" }, { name: "description", content: "Manage your GameHub profile." }],
  }),
    ...
    <AppShell>
      <div className="min-h-screen px-6 pt-32 pb-20 max-w-2xl mx-auto">
        ...
        <label className="block mb-6 glass-panel p-6">
          <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">Display Name</span>
          <input
            value={name}
          ...
          <div className="mt-3 flex justify-end">
            <NeonButton size="sm" onClick={() => updateProfile({ username: name.trim() || user.username })}>
              Save
            </NeonButton>
          </div>
        </label>
