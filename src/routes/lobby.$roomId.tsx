...
export const Route = createFileRoute("/lobby/$roomId")({
  head: () => ({
    meta: [{ title: "Lobby — GameHub" }, { name: "description", content: "Waiting room before the match starts." }],
  }),
  component: LobbyPage,
});
      ...
      <div className="min-h-screen px-6 pt-28 pb-32 max-w-5xl mx-auto">
        <header className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 mb-8">
          <div className="min-w-0">
            ...
            <h1 className="font-display text-4xl md:text-6xl italic uppercase truncate">{room.name}</h1>
            <div className="mt-3 flex flex-wrap items-center gap-3 text-[10px] font-mono uppercase tracking-widest text-white/40">
              <span className="inline-flex items-center gap-1.5"><Wifi className="size-3" /> {room.isLan ? "LAN" : "Online"}</span>
              <span>·</span>
          ...
          >
            <div className="text-[9px] font-mono uppercase tracking-[0.3em] text-white/40">Room Code</div>
            <div className="font-display text-2xl tracking-[0.3em] text-accent-cyan inline-flex items-center gap-2">
              {room.code}
              <Copy className="size-4" />
            </div>
            <div className="text-[9px] font-mono text-accent-cyan/60 h-3">{copied ? "COPIED" : "\u00a0"}</div>
          </button>
        </header>
        ...
        <div className="grid lg:grid-cols-[1fr_320px] gap-6">
                ...
                {room.players.map((p) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className={`text-[10px] font-mono uppercase tracking-widest ${p.ready ? "text-accent-cyan" : "text-white/40"}`}>
          ...
          <aside className="space-y-4">
            ...
            <div className="glass-panel p-5 text-xs font-mono text-white/50 leading-relaxed">
              Share the room code with friends, or fill seats with AI players. The host starts the match when everyone is ready.
            </div>
          </aside>
        </div>
      </div>
