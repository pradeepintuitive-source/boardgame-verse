...
export const Route = createFileRoute("/settings")({
  head: () => ({
    meta: [{ title: "Settings — GameHub" }, { name: "description", content: "Configure GameHub preferences." }],
  }),
      ...
      <div className="min-h-screen px-6 pt-32 pb-20 max-w-2xl mx-auto">
        ...
        <div className="space-y-4">
          {[
            ...
            <div key={f.key} className="glass-panel p-5 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4">
              <span className="text-sm font-mono uppercase tracking-widest text-white/70">{f.label}</span>
              <input
                type="range"
                min={0}
                max={1}
          ...
          {[
            ...
            <label key={f.key} className="glass-panel p-5 grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 cursor-pointer">
              <span className="text-sm font-mono uppercase tracking-widest text-white/70">{f.label}</span>
              <input
                type="checkbox"
                checked={s[f.key]}
