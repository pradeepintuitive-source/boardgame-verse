...
export const Route = createFileRoute("/join-room")({
  head: () => ({
    meta: [{ title: "Join Room — GameHub" }, { name: "description", content: "Join a GameHub room with a code." }],
  }),
        ...
        <form onSubmit={submit} className="w-full max-w-md glass-panel p-8">
          ...
          {!user && (
            <label className="block mb-4">
              <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">Your Name</span>
              <input
                value={name}
          ...
          <label className="block mb-2">
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">Room Code</span>
            <input
              value={code}
          ...
          <NeonButton type="submit" className="w-full mt-6">Join Game</NeonButton>
        </form>
      </div>
    </AppShell>
  );
