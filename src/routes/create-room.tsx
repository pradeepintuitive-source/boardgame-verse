...
export const Route = createFileRoute("/create-room")({
  head: () => ({
    meta: [{ title: "Create Room — GameHub" }, { name: "description", content: "Spin up a new GameHub room." }],
  }),
      ...
      <div className="min-h-screen px-6 pt-32 pb-20 max-w-2xl mx-auto">
        ...
        <form onSubmit={submit} className="glass-panel p-6 space-y-5">
          <label className="block">
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">Room Name</span>
            <input
              value={name}
          ...
          <div>
            <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">Game Type</span>
            <div className="mt-2 grid grid-cols-2 gap-3">
              {(["mafia", "monopoly"] as GameType[]).map((g) => (
                ...
                >
                  {g}
                  {g === "monopoly" && (
                    <div className="text-[9px] font-mono tracking-widest text-white/40 mt-1">SOON</div>
                  )}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="block">
              <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">Max Players</span>
              <input
                type="number" min={3} max={16} value={maxPlayers}
                onChange={(e) => setMax(Number(e.target.value))}
                className="mt-1 w-full bg-background border border-white/10 px-3 py-3 font-mono focus:border-accent-cyan outline-none"
              />
            </label>
            <label className="block">
              <span className="text-[10px] font-mono uppercase tracking-widest text-white/40">AI Players</span>
              <input
                type="number" min={0} max={maxPlayers - 1} value={ai}
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="flex items-center gap-3 p-3 border border-white/10 cursor-pointer">
              <input type="checkbox" checked={isPrivate} onChange={(e) => setPrivate(e.target.checked)} className="accent-[var(--accent-cyan)] size-4" />
              <span className="text-sm font-mono uppercase tracking-widest">Private</span>
            </label>
            <label className="flex items-center gap-3 p-3 border border-white/10 cursor-pointer">
              <input type="checkbox" checked={isLan} onChange={(e) => setLan(e.target.checked)} className="accent-[var(--accent-cyan)] size-4" />
              <span className="text-sm font-mono uppercase tracking-widest">LAN Mode</span>
            </label>
          </div>

          <div className="flex gap-3 pt-2">
            <NeonButton type="submit" className="flex-1">Create Room</NeonButton>
