...
export function Tile({ tile, prop, ownerColor, orientation, onClick, children, highlight }: Props) {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`font-mono uppercase tracking-tight leading-tight ${isCorner ? "text-[9px]" : "text-[7px]"} text-white/80`}>
          {tile.name}
        </div>
        {tile.price != null && (
        ...
        {prop?.mortgaged && (
          <div className="text-[7px] font-mono text-destructive">MTG</div>
        )}
        ...
        {ownerColor && (
          <div className="size-1.5 rounded-full mt-0.5" style={{ background: ownerColor, boxShadow: `0 0 6px ${ownerColor}` }} />
        )}
      </div>
      {children}
    </button>
