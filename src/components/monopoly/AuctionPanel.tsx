...
export function AuctionPanel({
  ...
  const isMyBid = currentBidderId === meId && me && !me.bankrupt && a.activePlayerIds.includes(meId);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="text-[10px] font-mono uppercase tracking-[0.4em] text-accent-pink mb-2">Live Auction</div>
        ...
        <div className="text-[10px] font-mono uppercase tracking-widest text-white/40">Highest Bid</div>
        ...
        {isMyBid ? (
          <div className="flex flex-col gap-2">
            ...
            <NeonButton variant="ghost" size="sm" onClick={onPass}>Pass</NeonButton>
          </div>
        ) : (
        ...
        <div className="mt-6 pt-4 border-t border-white/10 text-[10px] font-mono text-white/40">
          Remaining: {a.activePlayerIds.map((id) => state.players.find((p) => p.id === id)?.username).join(" · ")}
        </div>
      </motion.div>
    </motion.div>
  );
