...
## 6. Monopoly Uses REST for Normal Actions and STOMP Only for Auctions

- **Why it exists**: The backend returns the updated Monopoly state immediately from `POST /api/monopoly/{sessionId}/action`, while normal STOMP sends do not expose a structured per-message ACK/error channel.
- **Alternatives Considered**: Sending all Monopoly actions over STOMP and waiting solely for `/topic/game/{roomId}` broadcasts.
- **Trade-offs**:
